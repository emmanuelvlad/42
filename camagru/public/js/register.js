document.addEventListener("DOMContentLoaded", function(event) {
    ...
    form.addEventListener("submit", function(event) {
        if (!loading) {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open('POST', `${url}api/register`);
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    console.log(data);
                    if (data.success) {
                        addNotification(notifications, 'is-success', "You've been successfully registered, check your emails to confirm your registration!");
                        reset(username);
                        reset(email);
                        reset(password);
                        reset(password2);
